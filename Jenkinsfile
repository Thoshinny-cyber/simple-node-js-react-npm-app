pipeline{
    ...
    stages{
        ...
        stage('Docker Deploy'){
            steps{
              ansiblePlaybook credentialsId: 'dockerhost', disableHostKeyChecking: true, extras: "-e DOCKER_TAG=${DOCKER_TAG}", installation: 'ansible', inventory: 'dev.inv', playbook: 'deploy-docker.yml'
            }
        }
    }
}
